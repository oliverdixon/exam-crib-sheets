...
\textbf{Periodic Extensions}:
    If $f \colon [-\pi, \pi) \to \mathbb{R} $, then its
    \emph{periodic extension} $ \tilde{f} \colon \mathbb{R} \to \mathbb{R}$ is
    defined by $ \tilde{f}(x + 2\pi k) \coloneqq f(x)$ for $ k \in \mathbb{Z} $
    and $ -\pi \leq x < \pi$. &

\textbf{FCT (1)}:
    If $ f \colon [-\pi, \pi] \to \mathbb{R} $ is a PWCD$ {}^\dagger $ function,
    and $ \tilde{f} \colon \mathbb{R} \to \mathbb{R}$ is its $ 2\pi $-PE, then
...
\textbf{FCT (2)}:
    If $ \tilde{f} $ is continuous at $ x $, then $ S(x) = \tilde{f}(x) $.
...
\textbf{PT}:
    If $ f \colon [-\pi, \pi] \to \mathbb{R} $ is a PWCD with Fourier
    coefficients $ a_0 $, $ a_n $, and $ b_n $ for $ n \in \mathbb{N} $, then
        $ \int_{-\pi}^\pi
            f^2(x)
            \,\mathconst{d}x/\pi
        \equiv
...
\textbf{Imp. Diff.}:
    If $ y(x) $ is defined implicitly by $ f(x,y) = c $, then
...
\textbf{Laplacian}:
    ...
    If $ F(r, \theta) = f(r \cos \theta, r \sin \theta) $, then
        $ \Delta f \equiv \nabla^2 f =
